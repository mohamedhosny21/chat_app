...
import '../../../../../core/notifications_manager/data/models/notification_payload_model.dart';
...
class ChatRepository {
  ...
  Future<File> _saveFilePermanently(PlatformFile file) async {
    ...
    final sanitizedFileName = _sanitizeAndEncodeFileName(file.name);
    final newFile = File('${directory.path}/$sanitizedFileName');
  ...
  Future<String?> _uploadFileAndGetUrl(
    ...
    if (file != null) {
      final sanitizedFileName = _sanitizeAndEncodeFileName(fileName);

      final String filePath = '${_uuid.v1()}/$sanitizedFileName';
      final reference =
  ...
  String _sanitizeAndEncodeFileName(String fileName) {
    // Split the file name into base name and extension
    final dotIndex = fileName.lastIndexOf('.');

    // If a dot is found, this extracts everything before the dot.
    // If no dot is found (dotIndex == -1), the whole file name is considered the base name.
    final baseName =
        dotIndex == -1 ? fileName : fileName.substring(0, dotIndex);
    final extension = dotIndex == -1 ? '' : fileName.substring(dotIndex);

    // Sanitize the base name by replacing spaces with underscores and removing special characters except emojis
    final sanitizedBaseName =
        baseName.replaceAll(' ', '_').replaceAll(RegExp(r'[^\w\s-_]'), '');

    // Encode the sanitized base name to make it URI-safe, including emojis
    final encodedBaseName = Uri.encodeComponent(sanitizedBaseName);

    // Reassemble the file name with the encoded base name and the original extension
    final sanitizedAndEncodedFileName = '$encodedBaseName$extension';

    return sanitizedAndEncodedFileName;
  }
}
